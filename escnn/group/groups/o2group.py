...
class O2(Group):
    ...
    def _subgroup(self, id: Tuple[float, int]) -> Tuple[
        ...
        if id[0] is not None and id[1] == -1:
            ...
            parent_mapping = SO2ToO2Mapping(self)
            child_mapping = O2ToSO2Mapping(sg)
            ...
            parent_mapping = FlipToO2Mapping(axis, self)
            child_mapping = O2ToFlipMapping(axis, sg)
            ...
            parent_mapping = DnToO2Mapping(axis, self)
            child_mapping = O2ToDnMapping(axis, sg)
        else:
...
class O2ToSO2Mapping:
    def __init__(self, so2: Union[escnn.group.SO2, escnn.group.CyclicGroup]):
        self.so2 = so2
    ...
    def __call__(self, e: GroupElement):
        ...
        if flip == 0:
            try:
                return self.so2.element(rotation, 'radians')
            except ValueError:
                return None
        else:
            return None
...
class SO2ToO2Mapping:
    def __init__(self, o2):
        self.o2 = o2

    def __call__(self, e: GroupElement):
        assert isinstance(e.group, escnn.group.SO2) or isinstance(e.group, escnn.group.CyclicGroup)
        return self.o2.element(
            (0, e.to('radians')), 'radians'
        )
...
class O2ToFlipMapping:
    def __init__(self, axis: float, flip: escnn.group.CyclicGroup):
        assert isinstance(flip, escnn.group.CyclicGroup) and flip.order() == 2
        self.axis = axis
        self.flip = flip
    ...
    def __call__(self, e: GroupElement):
        ...
        elif f == 1 and utils.cycle_isclose(rot, self.axis, 2*np.pi):
...
class FlipToO2Mapping:
    def __init__(self, axis: float, o2: O2):
        self.axis = axis
        self.o2 = o2
    ...
    def __call__(self, e: GroupElement):
        ...
        if f == 0:
            return self.o2.identity
        else:
            return self.o2.element((1, self.axis))
...
class O2ToDnMapping:
    def __init__(self, axis: float, dn: escnn.group.DihedralGroup):
        assert isinstance(dn, escnn.group.DihedralGroup)
        self.axis = axis
        self.dn = dn
    ...
    def __call__(self, e: GroupElement):
        ...
        if utils.cycle_isclose(rot - f * self.axis, 0., 2. * np.pi / self.dn.rotation_order):
            return self.dn.element((f, int(round((rot - f * self.axis) * self.dn.rotation_order / (2. * np.pi)))))
        else:
            return None
...
class DnToO2Mapping:
    ...
    def __call__(self, e: GroupElement):
        ...
        return self.o2.element((f, rot * 2. * np.pi / e.group.rotation_order + f * self.axis))
