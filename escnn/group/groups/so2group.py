...
class CharacterBuilder:
    ...
    def __call__(self, e: GroupElement) -> float:
        k = self.k
        if k == 0:
            # Trivial representation
            return 1.
...
class ParentMapping:
    def __init__(self, G: SO2, order: int):
        self.G = G
        self.order = order

    def __call__(self, e: GroupElement):
        return self.G.element(e.to('int') * 2 * np.pi / self.order)


class ChildMapping:
    def __init__(self, sg: CyclicGroup):
        self.sg = sg

    def __call__(self, e: GroupElement):
        radians = e.to('radians')
        if not utils.cycle_isclose(radians, 0., 2. * np.pi / self.sg.order()):
            return None
        else:
            return self.sg.element(int(round(radians * self.sg.order() / (2. * np.pi))))
