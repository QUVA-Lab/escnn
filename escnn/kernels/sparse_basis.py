...
class SparseOrbitBasis(SteerableFiltersBasis):
    ...
    def __init__(self,
        ...
        Class which implements a steerable basis for a discrete homogeneous space ```X``` by embedding the finite
        ...
        coset associated with the identity, i.e. :math:`eH`, and the ``action`` of the group :math:`G` on :math:`\R^n`.
